function varargout = kjm_ctmr(varargin)
% KJM_CTMR M-file for kjm_ctmr.fig
%      KJM_CTMR, by itself, creates a new KJM_CTMR or raises the existing
%      singleton*.
%
%      H = KJM_CTMR returns the handle to a new KJM_CTMR or the handle to
%      the existing singleton*.
%
%      KJM_CTMR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in KJM_CTMR.M with the given input arguments.
%
%      KJM_CTMR('Property','Value',...) creates a new KJM_CTMR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before kjm_ctmr_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to kjm_ctmr_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help kjm_ctmr

% Last Modified by GUIDE v2.5 01-Apr-2011 15:02:18

% Begin initialization code - DO NOT EDIT

%   Created by:
%   D. Hermes, Dept of Neurology and Neurosurgery, University Medical Center Utrecht
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @kjm_ctmr_OpeningFcn, ...
                   'gui_OutputFcn',  @kjm_ctmr_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before kjm_ctmr is made visible.
function kjm_ctmr_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to kjm_ctmr (see VARARGIN)

% Choose default command line output for kjm_ctmr
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes kjm_ctmr wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = kjm_ctmr_OutputFcn(hObject, eventdata, handles) 
...
tempdata=spm_read_vols(handles.data.ctStruct);

%kjm changes -- scale data to have reasonable range, and maximum of 1 for imshow
%     tempdata=tempdata/(2*std(reshape(tempdata,1,[])));
%     tempdata(tempdata>1)=1;tempdata(tempdata<0)=0;
    tempdata=tempdata/(max(reshape(tempdata,1,[])));

    handles.data.ctData=tempdata;
...
%kjm change -- set settings for electrode selection
    set(handles.threshold,'String','0.75');
    set(handles.max_size,'String','3');
...
if isfield(handles.data,'elecMap') %if electrodes are there
    if handles.data.view==1
        rgbplaatje1=0.5*handles.data.ctData(:,:,currentZ)+(handles.data.elecMap(:,:,currentZ));
        imagesc(rgbplaatje1,...
            'Parent',handles.axes1,[0 1.1]);
    elseif handles.data.view==2
        rgbplaatje2=0.5*squeeze(handles.data.ctData(:,currentZ,:))+squeeze((handles.data.elecMap(:,currentZ,:)));
        imagesc(rgbplaatje2,...
            'Parent',handles.axes1,[0 1.1]);
    elseif handles.data.view==3     
        rgbplaatje3=0.5*squeeze(handles.data.ctData(currentZ,:,:))+squeeze((handles.data.elecMap(currentZ,:,:)));
        imagesc(rgbplaatje3,...
            'Parent',handles.axes1,[0 1.1]);
    end
    cm=colormap('gray');
    cm=[cm; 1 0 0; 1 0 0; 1 0 0];
    colormap(cm);
else
    if handles.data.view==1
        imagesc(handles.data.ctData(:,:,currentZ),...
            'Parent',handles.axes1,[0 1.1]);
        hold on
    elseif handles.data.view==2
        imagesc(squeeze(handles.data.ctData(:,currentZ,:)),...
            'Parent',handles.axes1,[0 1.1]);
        hold on
    elseif handles.data.view==3
        imagesc(squeeze(handles.data.ctData(currentZ,:,:)),...
...
%         imshow(rgbplaatje1,...
%             'Parent',handles.axes1,'XData',[0 1],'YData',[0 size(rgbplaatje1,1)/size(rgbplaatje1,2)]); 
%     elseif handles.data.view==2
...
%         imshow(rgbplaatje2,...
%             'Parent',handles.axes1,'XData',[0 1],'YData',[0 size(rgbplaatje2,1)/size(rgbplaatje2,2)]);
%     elseif handles.data.view==3     
%         rgbplaatje3=cat(3,0.5*squeeze(handles.data.ctData(currentZ,:,:))+squeeze((handles.data.elecMap(currentZ,:,:))),...
%             0.5*squeeze(handles.data.ctData(currentZ,:,:)),...
%             0.5*squeeze(handles.data.ctData(currentZ,:,:))+squeeze((handles.data.elecMap(currentZ,:,:))));
%         imshow(rgbplaatje3,...
%             'Parent',handles.axes1,'XData',[0 1],'YData',[0 size(rgbplaatje3,1)/size(rgbplaatje3,2)]);
%     end
% else
%     if handles.data.view==1
%         imshow(handles.data.ctData(:,:,currentZ),...
%             'Parent',handles.axes1,'XData',[0 1],'YData',[0 size(handles.data.ctData,1)/size(handles.data.ctData,2)]);
%         hold on
%     elseif handles.data.view==2
%         imshow(squeeze(handles.data.ctData(:,currentZ,:)),...
%             'Parent',handles.axes1,'XData',[0 1],'YData',[0 size(handles.data.ctData,1)/size(handles.data.ctData,3)]);
%         hold on
%     elseif handles.data.view==3
%         imshow(squeeze(handles.data.ctData(currentZ,:,:)),...
%             'Parent',handles.axes1,'XData',[0 1],'YData',[0 size(handles.data.ctData,2)/size(handles.data.ctData,3)]);
%         hold on
%     end
%     colormap('gray');
% end
