...
while c~='q'
    %%%%%%%%%%%%%%%%%%%%%%%plotting%%%%%%%%%%%%%%%%%%%%%%%%%%
    clf
    plot(freq_bins,squeeze(log(spectra(:,currchan,:))))
    ylabel('log power'),xlabel('frequency'),title(strcat([patient ' power spectra for channel ' num2str(currchan)]))
	legend(conditions,0)
    %%%%%%%%%%%%%%%%%%%%%%%advancing/parsing%%%%%%%%%%%%%%%%%
    waitforbuttonpress
    y=get(fp);
    ...
    elseif c=='s'%save this picture
        exportfig(gcf, strcat(cd,'\','pwr_',patient,'_',num2str(currchan),'_',num2str(min(freq_bins)),'_',num2str(max(freq_bins)),'.jpg'), 'format', 'jpeg', 'Renderer', 'painters', 'Color', 'cmyk', 'Resolution', 600, 'Width', 3, 'Height', 3); 
        exportfig(gcf, strcat(cd,'\','pwr_',patient,'_',num2str(currchan),'_',num2str(min(freq_bins)),'_',num2str(max(freq_bins)),'.eps'), 'format', 'eps', 'Renderer', 'painters', 'Color', 'cmyk', 'Resolution', 600, 'Width', 3, 'Height', 3); 
