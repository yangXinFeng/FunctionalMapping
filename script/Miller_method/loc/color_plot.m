function color_plot(electrodes,weights)
% function [electrodes]=color_plot_l(electrodes,weights)
% projects electrode locations onto their cortical spots on the 
% brain and plots about them using a gaussian kernel
...
%electrode radius for plotting beneath
% el_rad=1.15;
el_rad=2;
...
%     d=weights(i)*exp((-(b_x.^2+b_z.^2+b_y.^2).^.5)/gsp); %gaussian 
    d=weights(i)*and(and(b_x<el_rad,b_y<el_rad),b_z<el_rad);
    c=c+d';
end
c=(c'/max(c));
a=tripatch(cortex, '', c);
